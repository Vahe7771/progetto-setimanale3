...
*/
let random = alert(Math.floor(Math.random() * 21));
console.log(random);
...
console.log(isThisAnEmail("vahe.mykoyan"));
/* ESERCIZIO 7
...
function howManyDays(date) {
  const inputDte = new Date(date);
  const today = new Date();
  const timeDifference = today - inputDte;
  const daysDifference = Math.floor(timeDifference / (1000 * 60 * 60 * 24));
  return daysDifference;
}
const dateInput = "1990-11-02";
console.log(`Giorni trascorsi: ${howManyDays(dateInput)}`);
/* ESERCIZIO 10
...
const newestMovie = movies.reduce((prev, current) => {
...
function searchByTitle() {
  return movies.filter((movie) => movie.Title.includes("Lord"));
}
...
function searchAndDivide(str) {
  for (const movie of movies) {
    if (movie.Title.toLowerCase().includes(str.toLowerCase())) {
      results.match.push(movie);
    } else {
      results.unmatch.push(movie);
    }
  }
  return results;
}
...
function aggiungiSfondoRossoAiLink() {
  ...
  link.forEach(function (elemento) {
    elemento.style.backgroundColor = "red";
  });
}

document.addEventListener("DOMContentLoaded", function () {
  aggiungiSfondoRossoAiLink(a);
});
